...
import { fetchPopularMovies } from '../data/api/tmdbApiFetch';
...
const PopularScreen = (props:any) => {
  ...
  const fetchPopularCaller = async () => {
    try {
  ...
  return (
    <>
      {!connectionStatus ? (
        ...
        <LoadingScreen Message="Popular Movies Loading" />
      ) : (
        <SafeAreaView style={{
          flex: 1,
          ...
          />
          <HeaderBar title='Popular' isDark={isDarkMode} props={props} />
...

                {
                  !moreLoading ? (
                    <TouchableOpacity
                      onPress={async () => {
                        setMoreLoading(true)
                        const response = await fetchPopularMovies(page + 1);
