...
import {fetchTopRatedMovies} from '../data/api/tmdbApiFetch';
...
const TopRatedScreen = (props:any) => {
  ...
  return (
    <>
      {!connectionStatus ? (
        ...
        <LoadingScreen Message="Top Rated Movies Loading" />
      ) : (
        <SafeAreaView style={{
          flex: 1,
          ...
          />
          <HeaderBar title='Top Rated' isDark={isDarkMode} props={props} />
              ...
              <View style={{ alignSelf: 'center' }}>
                {
                  !moreLoading ? (
                    <TouchableOpacity
                      onPress={async () => {
                        setMoreLoading(true)
                        const response = await fetchTopRatedMovies(page + 1);
