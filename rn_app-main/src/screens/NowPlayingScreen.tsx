...
import { fetchNowShowingMovies} from '../data/api/tmdbApiFetch';
  ...
  return (
    <>
      {!connectionStatus ? (
        ...
        <LoadingScreen Message="Now Playing Movies Loading" />
      ) : (
        <SafeAreaView style={{
          flex: 1,
          ...
          />
          <HeaderBar title='Now Playing' isDark={isDarkMode} props={props} />
...

                {
                  !moreLoading ? (
                    <TouchableOpacity
                      onPress={async () => {
                        setMoreLoading(true)
                        const response = await fetchNowShowingMovies(page + 1);
