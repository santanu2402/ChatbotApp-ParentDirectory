...
import {fetchTrendingMovies} from '../data/api/tmdbApiFetch';
...
const TrendingScreen = (props:any) => {
  ...
  const fetchTrendingCaller = async () => {
    try {
  ...
  return (
    <>
      {!connectionStatus ? (
        ...
        <LoadingScreen Message="Trending Movies Loading" />
      ) : (
        <SafeAreaView style={{
          flex: 1,
          ...
          />
          <HeaderBar title='Trending' isDark={isDarkMode} props={props} />
...

                {
                  !moreLoading ? (
                    <TouchableOpacity
                      onPress={async () => {
                        setMoreLoading(true)
                        const response = await fetchTrendingMovies(page + 1);
